...
void Adafruit_ST77xx::displayInit(const uint8_t *addr) {
  ...
  while (numCommands--) {              // 循环读取命令
    cmd = pgm_read_byte(addr++);       // 命令
    numArgs = pgm_read_byte(addr++);   // 数据
    ms = numArgs & ST_CMD_DELAY;       // 判断是否延时
    numArgs &= ~ST_CMD_DELAY;          // 屏蔽出延迟位
    sendCommand(cmd, addr, numArgs);   //写入命令、数据、字节数
    addr += numArgs;
