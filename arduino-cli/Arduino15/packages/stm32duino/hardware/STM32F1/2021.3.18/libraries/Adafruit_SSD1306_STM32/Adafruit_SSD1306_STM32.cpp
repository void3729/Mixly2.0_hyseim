/*!
 * @file Adafruit_SSD1306_STM32.cpp
 *
 ...
 * Adafruit_GFX_STM32</a> being present on your system. Please make sure you have
...
/*!
    ...
    @return Adafruit_SSD1306_STM32 object.
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_STM32::Adafruit_SSD1306_STM32(uint8_t w, uint8_t h, TwoWire *twi,
  int8_t rst_pin, uint32_t clkDuring, uint32_t clkAfter) :
  Adafruit_GFX_STM32(w, h), spi(NULL), wire(twi ? twi : &Wire), buffer(NULL),
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_STM32::Adafruit_SSD1306_STM32(uint8_t w, uint8_t h,
  int8_t mosi_pin, int8_t sclk_pin, int8_t dc_pin, int8_t rst_pin,
  int8_t cs_pin) : Adafruit_GFX_STM32(w, h), spi(NULL), wire(NULL), buffer(NULL),
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_STM32::Adafruit_SSD1306_STM32(uint8_t w, uint8_t h, SPIClass *spi,
  int8_t dc_pin, int8_t rst_pin, int8_t cs_pin, uint32_t bitrate) :
  Adafruit_GFX_STM32(w, h), spi(spi ? spi : &SPI), wire(NULL), buffer(NULL),
...
/*!
    @brief  DEPRECATED constructor for SPI SSD1306 displays, using software
            ...
            one of the SSD1306_* size defines in Adafruit_SSD1306_STM32.h. New
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_STM32::Adafruit_SSD1306_STM32(int8_t mosi_pin, int8_t sclk_pin,
  int8_t dc_pin, int8_t rst_pin, int8_t cs_pin) :
  Adafruit_GFX_STM32(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT), spi(NULL), wire(NULL),
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_STM32::Adafruit_SSD1306_STM32(int8_t dc_pin, int8_t rst_pin,
  int8_t cs_pin) : Adafruit_GFX_STM32(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT),
...
/*!
    @brief  DEPRECATED constructor for I2C SSD1306 displays. Provided for
            ...
            defines in Adafruit_SSD1306_STM32.h. New code should NOT use this.
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_STM32::Adafruit_SSD1306_STM32(int8_t rst_pin) :
  Adafruit_GFX_STM32(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT), spi(NULL), wire(&Wire),
  buffer(NULL), mosiPin(-1), clkPin(-1), dcPin(-1), csPin(-1),
  rstPin(rst_pin) {
}

/*!
    @brief  Destructor for Adafruit_SSD1306_STM32 object.
*/
Adafruit_SSD1306_STM32::~Adafruit_SSD1306_STM32(void) {
  if(buffer) {
    free(buffer);
    buffer = NULL;
  }
...
inline void Adafruit_SSD1306_STM32::SPIwrite(uint8_t d) {
  if(spi) {
    (void)spi->transfer(d);
  } else {
...
void Adafruit_SSD1306_STM32::ssd1306_command1(uint8_t c) {
  if(wire) { // I2C
...
void Adafruit_SSD1306_STM32::ssd1306_commandList(const uint8_t *c, uint8_t n) {
  if(wire) { // I2C
    wire->beginTransmission(i2caddr);
    WIRE_WRITE((uint8_t)0x00); // Co = 0, D/C = 0
    uint8_t bytesOut = 1;
...
/*!
    ...
    @return None (void).
*/
void Adafruit_SSD1306_STM32::ssd1306_command(uint8_t c) {
  TRANSACTION_START
  ssd1306_command1(c);
  TRANSACTION_END
}
...
*/
boolean Adafruit_SSD1306_STM32::begin(uint8_t vcs, uint8_t addr, boolean reset,
  boolean periphBegin) {
  ...
  if(wire) { // Using I2C
    ...
    //pinMode(PA3, OUTPUT); // Same for chip select
#ifdef HAVE_PORTREG
...
/*!
    @brief  Set/clear/invert a single pixel. This is also invoked by the
            Adafruit_GFX_STM32 library in generating many higher-level graphics
            primitives.
    @param  x
...
*/
void Adafruit_SSD1306_STM32::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void Adafruit_SSD1306_STM32::clearDisplay(void) {
  memset(buffer, 0, WIDTH * ((HEIGHT + 7) / 8));
}

/*!
    @brief  Draw a horizontal line. This is also invoked by the Adafruit_GFX_STM32
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void Adafruit_SSD1306_STM32::drawFastHLine(
...
void Adafruit_SSD1306_STM32::drawFastHLineInternal(
  int16_t x, int16_t y, int16_t w, uint16_t color) {
...
/*!
    @brief  Draw a vertical line. This is also invoked by the Adafruit_GFX_STM32
            library in generating many higher-level graphics primitives.
    @param  x
            Column of display -- 0 at left to (screen width -1) at right.
    @param  y
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void Adafruit_SSD1306_STM32::drawFastVLine(
...
void Adafruit_SSD1306_STM32::drawFastVLineInternal(
...
*/
boolean Adafruit_SSD1306_STM32::getPixel(int16_t x, int16_t y) {
  if((x >= 0) && (x < width()) && (y >= 0) && (y < height())) {
    // Pixel is in-bounds. Rotate coordinates if needed.
    switch(getRotation()) {
     case 1:
...
*/
uint8_t *Adafruit_SSD1306_STM32::getBuffer(void) {
  return buffer;
}
...
*/
void Adafruit_SSD1306_STM32::display(void) {
  TRANSACTION_START
...
void Adafruit_SSD1306_STM32::startscrollright(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void Adafruit_SSD1306_STM32::startscrollleft(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void Adafruit_SSD1306_STM32::startscrolldiagright(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void Adafruit_SSD1306_STM32::startscrolldiagleft(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
/*!
    ...
    @return None (void).
*/
void Adafruit_SSD1306_STM32::stopscroll(void) {
  TRANSACTION_START
...
*/
void Adafruit_SSD1306_STM32::invertDisplay(boolean i) {
  TRANSACTION_START
...
*/
void Adafruit_SSD1306_STM32::dim(boolean dim) {
  uint8_t contrast;
...
long Adafruit_SSD1306_STM32::getAddr(uint16_t str)
{
...
byte Adafruit_SSD1306_STM32::revBit(byte input)
{
  byte output = 0;
  for (int i = 0;  i < 8;  i++)
  {
...
void Adafruit_SSD1306_STM32::fetchBitmap16(long address, byte *p)
{
  ...
  //Serial.println("");
  //digitalWrite(PA3, HIGH);//反选通字库
  SSD1306_SELECT;
  SPI_TRANSACTION_END;
}

void Adafruit_SSD1306_STM32::fetchBitmap32(long address, byte *p)
{
  byte hb = (byte)((address & 0x00ff0000) >> 16);
  byte mb = (byte)((address & 0x0000ff00) >> 8);
  byte lb = (byte)(address & 0x000000ff);
...
void Adafruit_SSD1306_STM32::drawV8P(int x, int y, byte b)
{
...
void Adafruit_SSD1306_STM32::draw8x16(int x, int y, byte *b)
{
  int i;
  for (i = 0; i < 8; i++)
  {
...
void Adafruit_SSD1306_STM32::draw16x16(int x, int y, byte *b)
{
  int i;
  for (i = 0; i < 16; i++)
  {
...
void Adafruit_SSD1306_STM32::displayStr(int x, int y, int len, unsigned char *c)
{
  int i;
  for (i = 0; i < len; i++)
  {
...
void Adafruit_SSD1306_STM32::displayStrC(int row, int len, unsigned char *c)
{
    int x=64-((len*8)/2);
