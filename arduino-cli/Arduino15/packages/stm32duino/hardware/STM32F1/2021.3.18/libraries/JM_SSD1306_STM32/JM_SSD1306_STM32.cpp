...
JM_SSD1306_STM32::JM_SSD1306_STM32(SPIClass *spi) :
  Adafruit_GFX_STM32(128, 64), spi(spi ? spi : &SPI), wire(NULL), buffer(NULL),
  mosiPin(-1), clkPin(-1), dcPin(PA13), csPin(PA14), rstPin(-1) {
#ifdef SPI_HAS_TRANSACTION
  spiSettings = SPISettings(8000000UL, MSBFIRST, SPI_MODE0);
#endif
}
/*!
    @brief  Destructor for JM_SSD1306_STM32 object.
*/
JM_SSD1306_STM32::~JM_SSD1306_STM32(void) {
  if(buffer) {
    free(buffer);
    buffer = NULL;
  }
...
inline void JM_SSD1306_STM32::SPIwrite(uint8_t d) {
  if(spi) {
    (void)spi->transfer(d);
  } else {
...
void JM_SSD1306_STM32::ssd1306_command1(uint8_t c) {
...
void JM_SSD1306_STM32::ssd1306_commandList(const uint8_t *c, uint8_t n) {
...
/*!
    ...
    @return None (void).
*/
void JM_SSD1306_STM32::ssd1306_command(uint8_t c) {
  TRANSACTION_START
  ssd1306_command1(c);
  TRANSACTION_END
}
...
*/
boolean JM_SSD1306_STM32::begin(uint8_t vcs, uint8_t addr, boolean reset,
  boolean periphBegin) {
...
*/
void JM_SSD1306_STM32::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void JM_SSD1306_STM32::clearDisplay(void) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void JM_SSD1306_STM32::drawFastHLine(
...
void JM_SSD1306_STM32::drawFastHLineInternal(
  int16_t x, int16_t y, int16_t w, uint16_t color) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void JM_SSD1306_STM32::drawFastVLine(
...
void JM_SSD1306_STM32::drawFastVLineInternal(
...
*/
boolean JM_SSD1306_STM32::getPixel(int16_t x, int16_t y) {
  if((x >= 0) && (x < width()) && (y >= 0) && (y < height())) {
    // Pixel is in-bounds. Rotate coordinates if needed.
    switch(getRotation()) {
     case 1:
...
*/
uint8_t *JM_SSD1306_STM32::getBuffer(void) {
  return buffer;
}
...
*/
void JM_SSD1306_STM32::display(void) {
  TRANSACTION_START
...
void JM_SSD1306_STM32::startscrollright(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void JM_SSD1306_STM32::startscrollleft(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void JM_SSD1306_STM32::startscrolldiagright(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void JM_SSD1306_STM32::startscrolldiagleft(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
/*!
    ...
    @return None (void).
*/
void JM_SSD1306_STM32::stopscroll(void) {
  TRANSACTION_START
...
*/
void JM_SSD1306_STM32::invertDisplay(boolean i) {
  TRANSACTION_START
...
*/
void JM_SSD1306_STM32::dim(boolean dim) {
  uint8_t contrast;
