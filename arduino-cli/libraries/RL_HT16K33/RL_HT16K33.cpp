...
/*
*函数名称：	 uint16_t _flip_uint16(uint16_t in)
*函数功能：	 像素点左右反转
...
void HT16K33::writeDisplay(void) {
  ...
  for (uint8_t i=0; i<8; i++) {
	  if(hf_flip){		  
    displaybuffer[i]=_flip_uint16(displaybuffer[i]);
	  }
...
void HT16K33::setHf_flip(uint8_t x) {
  hf_flip = x;  
}
/*
*函数名称：	 clear(void)
...
void HT16K33::drawStr(String s){
	...
	if(a==1){
		clear();
		if(hf_flip==1)setCursor(9,0);
		else setCursor(6,0);
		print(s);
		writeDisplay();
		delay(100);
	}
	else if(a==2&&hf_flip!=1){
		clear();
		setCursor(3,0);
		print(s);
		writeDisplay();
