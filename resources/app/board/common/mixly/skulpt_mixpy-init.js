...
function init() {
    ...
    $.post("../../event.php",{"data":JSON.stringify(window.actionArrayRecord),"fileName":fileName,"type":'circuit'},function (res) {
         if(res.code===50){
            layer.msg(res.msg);
    ...
    */
    ...
    if (Mixly.Env.isClient) {
        ...
        Mixly.Title.updateTitle("Mixly 2.0 For Python 3 Lite");
    } else {
        getid("py-saveas").style.display = "none";
        Mixly.Title.updateTitle("mixpy for Mixly_Lite");
    }
    */
...
function showTag() {
    ...
    tag_select('view_btn', 'view_btn');
    tag_select('save_ser_btn', 'save_ser');
    tag_select('upload_btn', 'upload');
}
