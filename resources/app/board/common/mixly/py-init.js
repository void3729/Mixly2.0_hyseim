...
function tabClick(clickedName) {
    ...
    if (clickedName == 'blocks') {
       ...
       */
        ...
        if (Mixly.Config.BOARD?.pythonToBlockly) {
...
function renderContent() {
    ...
    if (content.id == 'content_blocks') {
        ...
        // 当为mixpy页面时，检测到input()函数则在代码最上方加一行
        if (Mixly.Config.BOARD.boardIndex.indexOf("/python_mixpy/") != -1) {
            try {
...
function init() {
    ...
    editor_side_code.setScrollSpeed(0.8);
    ...
    editor.commands.addCommands([
        {
            ...
            exec: function (editor) {
                var size = parseInt(editor.getFontSize(), 10) || 12;
                editor.setFontSize(size + 1);
                editor_side_code.setFontSize(size + 1);
            }
        }, {
            ...
            exec: function (editor) {
                var size = parseInt(editor.getFontSize(), 10) || 12;
                editor.setFontSize(Math.max(size - 1 || 1));
                editor_side_code.setFontSize(Math.max(size - 1 || 1));
            }
        }, {
            name: "resetFontSize",
            ...
            exec: function (editor) {
                ...
                editor_side_code.setFontSize(12);
            }
        }
    ]);
    //动态生成按钮元素
    ...
    //动态生成按钮元素
    //$('#div_inout_middle').append('<select id="select_com" style="position:absolute;right:120px;width:100px;height:20px;top:2px;"></select>');
    //$('#div_inout_middle').append('<button id="button_update" onclick="update_select_com()" style="position:absolute;right:70px;height:20px;top:2px;">刷新</button>');
    //$('#div_inout_middle').append('<button id="button_connect" onclick="connect_com()" style="position:absolute;right:20px;height:20px;top:2px;">打开</button>');
    ...
    if (typeof Mixly.Nav.navBtnConfig.leftBtn === "object") {
        ...
        for (let i = 0; i < len; i++) {
            ...
            nav_item_id.push('li_' + id);
        }
        endBtnId = Mixly.Nav.navBtnConfig.leftBtn[len - 1].id;
    }
    //获取左半面按钮最右端离index最左端的距离
    MixlyBlockly.layerOffsetLeft = getid(endBtnId).offsetParent.offsetLeft + getid(endBtnId).offsetParent.offsetWidth;
    ...
    var onresize = function (e) {
        ...
        var boardSelector = getid("boardSelector");
        var li_operate = getid("li_operate");
        var ul_nav = getid("nav");

        if (Code.LANG === "en") {
            getid("boardSelector-div").style.right = "160px";
        } else {
            getid("boardSelector-div").style.right = "150px";
        }

        if (boardSelector.offsetParent.offsetLeft < MixlyBlockly.layerOffsetLeft + 105) {
            if (boardSelector.offsetParent.offsetLeft < li_operate.offsetLeft + li_operate.offsetWidth + 105)
            ...
            var copyright_width = boardSelector.offsetParent.offsetLeft - (li_operate.offsetLeft + li_operate.offsetWidth);
            copyright.style.width = copyright_width;
            copyright.style.left = li_operate.offsetLeft + li_operate.offsetWidth;
            copyright.style.textAlign = "center";
            copyright.style.top = (ul_nav.clientHeight - copyright.offsetHeight) / 2;
        } else {
            copyright.style.display = "";
            ...
            var copyright_width = boardSelector.offsetParent.offsetLeft - MixlyBlockly.layerOffsetLeft;
            copyright.style.width = copyright_width;
            copyright.style.left = MixlyBlockly.layerOffsetLeft;
            copyright.style.textAlign = "center";
            copyright.style.top = (ul_nav.clientHeight - copyright.offsetHeight) / 2;
        }

        if (getid(endBtnId).offsetParent) {
            MixlyBlockly.layerOffsetLeft = getid(endBtnId).offsetParent.offsetLeft + getid(endBtnId).offsetParent.offsetWidth;
        }

        if (Mixly.StatusBar.selected) {
            if (now_visual_height > document.body.clientHeight) {
                var iT = 0.8;
                var percent = (document.body.clientHeight - ul_nav.clientHeight) * iT;
    ...
    if (String(Mixly.Config.BOARD?.thirdPartyBoard).toLowerCase() == "true") {
    ...
    if (Mixly.Config.software?.socketServer?.enabled === 'true')
