    ...
    runner.usart.onByteTransmit = (value) => {
        ...
        */
        if ((value & 0x80) == 0x00) {
            if (chineseStr !== "") {
                try {
                    let dataArr = String.fromCharCode(value).match(/\w/g);
                    if (dataArr && dataArr.length > 0) {
                        chineseStr += String.fromCharCode(value);
                    } else {
                        serialOutputText.textContent += strDecode(chineseStr);
            ...
            receiveBuff.push(value);
            if (receiveBuff.length >= buffLength) {
                serialOutputText.textContent += Buffer.from(receiveBuff);
                receiveBuff = [];
            }
        } else {
            let dataNum = value & 0xe0;
            switch (dataNum) {
                case 0xfc:
                    buffLength = 6;
                    break;
...
function buildHex() {
    var simulateData = Mixly.Modules.fs.readFileSync(Mixly.Env.clientPath + "/mixlyBuild/testArduino.ino.hex", 'utf-8');
    return {
        "hex": simulateData,
        "eep": ""
    }
