...
Code.changeEditorTheme_light = function () {
    ...
    $("body").removeClass("mixly-bg-cyan");
    $("body").addClass("mixly-bg-green");
    ...
    try {
        ...
        if (Mixly.StatusBar.Ace != null) {
            Mixly.StatusBar.Ace.setOption("theme", "ace/theme/xcode");
        }
        if (Mixly.StatusBar?.portName) {
            for (let i = 0, length = Mixly.StatusBarPort.portName.length; i < length; i++) {
                Mixly.StatusBarPort.portAce[Mixly.StatusBarPort.portName[i]].setOption("theme", "ace/theme/xcode");
            }
        }
        //JSFuncs.saveEditorTheme(theme);
    } catch (e) {
...
Code.changeEditorTheme_dark = function () {
    ...
    $("body").removeClass("mixly-bg-green");
    $("body").addClass("mixly-bg-cyan");
    ...
    try {
        ...
        if (Mixly.StatusBar.Ace != null) {
            Mixly.StatusBar.Ace.setOption("theme", "ace/theme/terminal");
        }
        if (Mixly.StatusBar?.portName) {
            for (let i = 0, length = Mixly.StatusBarPort.portName.length; i < length; i++) {
                Mixly.StatusBarPort.portAce[Mixly.StatusBarPort.portName[i]].setOption("theme", "ace/theme/terminal");
            }
        }
        //JSFuncs.saveEditorTheme(theme);
    } catch (e) {
...
if (Mixly.Config.BOARD?.thirdPartyBoard) {
...
// 动态加载用户配置的语言文件夹
if (Mixly.Config.BOARD?.langFileDir) {
    if (typeof Mixly.Config.BOARD.langFileDir === "object") {
        for (let i = 0, len = Mixly.Config.BOARD.langFileDir.length; i < len; i++) {
            Mixly.ScriptLoader.loadScript(Mixly.Config.BOARD.langFileDir[i] + '/' + Code.LANG + '.js');
        }
    } else {
        Mixly.ScriptLoader.loadScript(Mixly.Config.BOARD.langFileDir + '/' + Code.LANG + '.js');
    }
}

function loadThirdPartyLanJs() {
    if (!Mixly.Env.isClient) return;

    //动态加载板卡目录msg
    //if (Mixly.Modules.fs.existsSync(Mixly.Env.indexPath + "/language/" + Code.LANG + ".js")) {
    //    Mixly.ScriptLoader.loadScript('language/' + Code.LANG + '.js');
    //}
        ...
        if (Mixly.Modules.fs.existsSync(rootPath + "/libraries/ThirdParty/")) {
            ...
            for (var i = 0; i < libDir.length; i++) {
                if (Mixly.Modules.fs.existsSync(rootPath + "/libraries/ThirdParty/" + libDir[i] + "/")) {
                    try {
                        if (Mixly.Modules.fs.existsSync(rootPath + "/libraries/ThirdParty/" + libDir[i] + "/language/" + Code.LANG + ".js")) {
                            Mixly.ScriptLoader.loadScript('libraries/ThirdParty/' + libDir[i] + '/language/' + Code.LANG + '.js');
                            Mixly.Env.thirdPartyJS.push('libraries/ThirdParty/' + libDir[i] + '/language/' + Code.LANG + '.js');
                        }
                        if (Mixly.Modules.fs.existsSync(rootPath + "/libraries/ThirdParty/" + libDir[i] + "/language/")) {
                            var langDir = Mixly.Modules.fs.readdirSync(rootPath + "/libraries/ThirdParty/" + libDir[i] + "/language/");
                            for (var j = 0; j < langDir.length; j++) {
                                if (Mixly.Modules.fs.existsSync(rootPath + "/libraries/ThirdParty/" + libDir[i] + "/language/" + langDir[j] + "/" + Code.LANG + ".js")) {
                                    Mixly.ScriptLoader.loadScript('libraries/ThirdParty/' + libDir[i] + '/language/' + langDir[j] + '/' + Code.LANG + '.js');
                                    Mixly.Env.thirdPartyJS.push('libraries/ThirdParty/' + libDir[i] + '/language/' + langDir[j] + '/' + Code.LANG + '.js');
                                }
                            }
                        }
                    } catch (e) {
