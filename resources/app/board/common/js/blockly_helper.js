...
function backup_blocks() {
    ...
    if ('localStorage' in window && window['localStorage'] != null && document.getElementById('changemod_btn').value == 0) {
        window.localStorage.setItem(Mixly.Config.BOARD.boardName + ".code", editor.getValue());
...
function restore_blocks() {
    ...
    if ('localStorage' in window && window['localStorage'] != null
        ...
        if ('localStorage' in window && window['localStorage'] != null
            ...
            try {
                ...
                setTimeout(function () {
                    tabClick('arduino');
                    editor.setValue(window.localStorage[Mixly.Config.BOARD.boardName + ".code"], -1);
                }, 500);
                //tabClick('arduino');
            } catch (e) {
                console.log(e);
    ...
    if ('localStorage' in window && window['localStorage'] != null
        ...
        if (Mixly.Modules.fs.existsSync(loadPath)) {
            ...
            Mixly.Electron.File.setPath(loadPath);
            try {
                Mixly.Electron.File.nowPath = loadPath.substring(0, loadPath.lastIndexOf("\\") + 1);
            } catch (e) {
                console.log(e);
            }
        }
...
function save() {
    ...
    if (document.getElementById("boardSelector").value == "") {
        ...
        var fn = document.getElementById("boardSelector").value + '.mix'
    saveAs(blob, fn);
}
...
function auto_save_and_restore_blocks() {
    ...
    if (BOARD?.nav?.save?.hex && BOARD.nav.save.py)
        $('.loading').remove();
    else
...
mixlyjs.saveInoFileAs = function (f) {
    ...
    if (!document.getElementById("boardSelector").value) {
        ...
        var fn = document.getElementById("boardSelector").value + '.ino'
    }
    var blob = new Blob(
        [xmlCodes],
        { type: 'text/plain;charset=utf-8' });
...
mixlyjs.savePyFileAs = function (f) {
    ...
    if (!document.getElementById("boardSelector").value) {
        ...
        var fn = document.getElementById("boardSelector").value + '.py'
    }
    var blob = new Blob(
        [xmlCodes],
        { type: 'text/plain;charset=utf-8' });
...
mixlyjs.compileMicrobitPy = function () {
    if (!document.getElementById("boardSelector").value) {
        ...
        var fn = document.getElementById("boardSelector").value + '.hex'
    }
...
mixlyjs.saveBlockImg = function () {
    ...
    img.onload = function () {
        ...
        if (document.getElementById("boardSelector").value == "") {
            ...
            var fn = document.getElementById("boardSelector").value + '.png'
