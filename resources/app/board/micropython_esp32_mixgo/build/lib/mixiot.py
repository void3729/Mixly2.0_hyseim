...
def init_MQTT_client(address, username, password, project, callback):
    client = MQTTClient(hexlify(machine.unique_id()), address, 1883, username, password, project)
    client.set_callback(callback)
    MQTT_USR_PRJ = "{}/{}/".format(username,project)
    client.set_last_will(topic=MQTT_USR_PRJ+WILL_TOPIC, msg=mqtt_client.client_id, qos=2)
    if client.connect()==0:
        client.publish(MQTT_USR_PRJ+ADDITIONAL_TOPIC, mqtt_client.client_id, qos=1)
    # client.connect()
...
class MQTTClient:
    ...
    def connect(self, clean_session=True):
        ...
        if self.username is not None:
            msg[1] += 2 + len(self.username) + 2 + len(self.password)
            msg[9] |= 0xC0
        if self.keepalive:
