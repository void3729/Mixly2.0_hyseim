...
pbc.assignD.get('RC522')['check_assign'] = function (py2block, node, targets, value) {
    if(value.func._astname != "Attribute" || value.func.value._astname != "Name"){
        return false;
    }
    var moduleName = py2block.Name_str(value.func.value);
    var funcName = py2block.identifier(value.func.attr);
    if (value._astname === "Call" && moduleName === "rc522"
        && funcName === "RC522" && value.args.length === 2)
        return true;
    return false;
}

pbc.assignD.get('RC522')['create_block'] = function (py2block, node, targets, value) {
    var sub = py2block.convert(node.targets[0]);
    var spisub = py2block.convert(value.args[0]);
    var pinsub = py2block.convert(value.args[1]);
     //注意：赋值语句里，即使图形块上下可接，也不需要加[]
    return block('rfid_use_spi_init', node.lineno, {}, {
        "SPISUB": spisub,
        "PINSUB": pinsub,
        "SUB": sub
    });
...
pbc.objectFunctionD.get('read_card')['rfid'] = function (py2block, func, args, keywords, starargs, kwargs, node) {
    if (args.length !== 0) {
        throw new Error("Incorrect number of arguments");
    }
    var objblock = py2block.convert(func.value);
    return block("rfid_read", func.lineno, {}, {
        "SUB": objblock
    }, {
        "inline": "true"
    });
...
pbc.objectFunctionD.get('write_card')['rfid'] = function (py2block, func, args, keywords, starargs, kwargs, node) {
    if (args.length !== 1) {
        throw new Error("Incorrect number of arguments");
    }
    var objblock = py2block.convert(func.value);
    var args0 = py2block.convert(args[0]);
    return [block("rfid_write", func.lineno, {}, {
        "SUB": objblock,
        "CONTENT": args0
    }, {
        "inline": "true"
